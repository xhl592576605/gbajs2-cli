export default class GameBoyAdvanceAudio {
	constructor() {
		this.context = null;
			...
			if (this.enableChannel3 && this.playingChannel3) {
				if (cycles >= this.channel3Next) {
					if (this.channel3Write) {
						var waveSample = this.waveData[this.channel3Pointer >> 1];
					this.channel3Sample = (((waveSample >> ((this.channel3Pointer & 1) << 2)) & 0xF) - 0x8) / 8;
			...
			if (this.enableChannel4 && this.playingChannel4) {
				if (this.channel4.timed && cycles >= this.channel4.end) {
					...
					if (cycles >= this.channel4.next) {
						...
						var noiseBit = this.channel4.lfsr & 1;
						this.channel4.lfsr |=
							(((this.channel4.lfsr >> 1) & 1) ^ noiseBit) <<
							(this.channel4.width - 1);
						this.channel4.next += this.channel4.interval;
						this.channel4.sample =
							(noiseBit - 0.5) * 2 * this.channel4.volume;
					}
	...
	sample() {
		...
		if (channel.playing) {
			var square0 = channel.sample * this.soundRatio * this.PSG_MAX;
		...
		channel = this.squareChannels[1];
		if (channel.playing) {
			var square1 = channel.sample * this.soundRatio * this.PSG_MAX;
		...
		if (this.playingChannel4) {
			var noiseSample = this.channel4.sample * this.soundRatio * this.PSG_MAX;
			if (this.enabledLeft & 0x8) {
				sampleLeft += noiseSample;
			}
			if (this.enabledRight & 0x8) {
				sampleRight += noiseSample;
			}
		}

		if (this.enableChannelA) {
			var fifoASample = this.fifoASample * this.FIFO_MAX * this.ratioChannelA;
			if (this.enableLeftChannelA) {
				sampleLeft += fifoASample;
			}
			if (this.enableRightChannelA) {
				sampleRight += fifoASample;
			}
		}

		if (this.enableChannelB) {
			var fifoBSample = this.fifoBSample * this.FIFO_MAX * this.ratioChannelB;
			if (this.enableLeftChannelB) {
				sampleLeft += fifoBSample;
			}
			if (this.enableRightChannelB) {
				sampleRight += fifoBSample;
			}
		}
	...
	getAvailableSamples() {
		return (
			(this.samplePointer - this.outputPointer + this.maxSamples) &
			this.sampleMask
		);
	}
	pullSamples(frameCount) {
		if (!frameCount) {
			frameCount = this.bufferSize;
		}
		if (!this.masterEnable) {
			return new Float32Array(0);
		}
		var available = this.getAvailableSamples();
		if (!available) {
			return new Float32Array(0);
		}
		var samples = Math.min(frameCount, available);
		var result = new Float32Array(samples * 2);
		for (var i = 0; i < samples; ++i) {
			var index = (this.outputPointer + i) & this.sampleMask;
			result[i * 2] = this.buffers[0][index];
			result[i * 2 + 1] = this.buffers[1][index];
		}
		this.outputPointer =
			(this.outputPointer + samples) & this.sampleMask;
		return result;
	}
}
