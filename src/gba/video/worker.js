...
const port = typeof parentPort !== "undefined" ? parentPort : null;
...
var workerCaller = {
	finishDraw: function (pixelData) {
		postMessage({ type: "finish", frame: currentFrame, backing: null });
	}
};
...
var handlers = {
	...
	start: function(data) {
		var backingInfo = data.backing;
		proxyBacking = {
			width: backingInfo.width,
			height: backingInfo.height,
			data: new Uint8ClampedArray(backingInfo.buffer)
		};
		video.setBacking(proxyBacking);
	},
	...
	finish: function(data) {
		...
		video.finishDraw(workerCaller);
	},
};

registerMessageHandler(function (message) {
