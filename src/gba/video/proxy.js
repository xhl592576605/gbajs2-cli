...
class GameBoyAdvanceRenderProxy {
	constructor() {
		...
		this.worker = new NodeWorker(workerUrl, { type: "module" });
	...
	setBacking(backing) {
		if (!(backing && backing.data && backing.data.buffer)) {
			throw new Error("Invalid backing passed to render proxy");
		}
		if (
			typeof SharedArrayBuffer === "undefined" ||
			!(backing.data.buffer instanceof SharedArrayBuffer)
		) {
			throw new Error(
				"Render proxy requires SharedArrayBuffer-backed frame buffers"
			);
		}
