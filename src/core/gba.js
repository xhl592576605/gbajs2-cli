...
const { ARMCore } = require('./core');
const { GameBoyAdvanceMMU } = require('./mmu');
const { GameBoyAdvanceInterruptHandler } = require('./irq');
const { GameBoyAdvanceIO } = require('./io');
const { GameBoyAdvanceAudio } = require('./audio');
const { GameBoyAdvanceVideo } = require('./video');
const { GameBoyAdvanceKeypad } = require('./keypad');
const { GameBoyAdvanceSIO } = require('./sio');
...
class GameBoyAdvance {
	...
	setCanvas(canvas) {
		// In Node.js environment, we directly set the canvas
		this.setCanvasDirect(canvas);
	}
	setCanvasDirect(canvas) {
		// In Node.js environment, canvas context is handled differently
		this.context = canvas.getContext ? canvas.getContext("2d") : canvas;
	...
	loadRomFromFile(romFile, callback) {
		// In Node.js environment, we use fs to read files
		// This method should be replaced with direct file reading in Node.js
		console.warn("loadRomFromFile is not supported in Node.js environment. Use setRom with Buffer instead.");
		if (callback) {
			callback(false);
		}
	}
	...
	loadSavedataFromFile(saveFile) {
		// In Node.js environment, we use fs to read files
		// This method should be replaced with direct file reading in Node.js
		console.warn("loadSavedataFromFile is not supported in Node.js environment. Use setSavedata with Buffer instead.");
	}
	...
	downloadSavedata() {
		...
		// In Node.js environment, we save to file system instead of opening in browser
		console.warn("downloadSavedata is not implemented for Node.js environment. Use file system instead.");
		return null;
	}
	storeSavedata() {
		var sram = this.mmu.save;
		try {
			// In Node.js environment, we use file system instead of localStorage
			// This method should be implemented with fs.writeFile in Node.js
			console.warn("storeSavedata is not implemented for Node.js environment. Use file system instead.");
		} catch (e) {
	...
	retrieveSavedata() {
		try {
			// In Node.js environment, we use file system instead of localStorage
			// This method should be implemented with fs.readFile in Node.js
			console.warn("retrieveSavedata is not implemented for Node.js environment. Use file system instead.");
		} catch (e) {
