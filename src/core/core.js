...
const { ARMCoreArm } = require('./arm');
const { ARMCoreThumb } = require('./thumb');
...
class ARMCore {
	...
	fetchPage(address) {
		...
		if (region == this.pageRegion) {
			if (this.page && pageId == this.pageId && !this.page.invalid) {
				return;
			}
			this.pageId = pageId;
		} else {
	...
	loadInstructionArm(address) {
		...
		if (!this.page) {
			...
			instruction.writesPC = false;
			instruction.next = null;
			instruction.page = null;
			instruction.address = 0;
			instruction.opcode = 0xe1a00000; // mov r0, r0 (nop-like)
			return instruction;
		}
	...
	loadInstructionThumb(address) {
		...
		if (!this.page) {
			...
			instruction.page = null;
			instruction.address = 0;
			instruction.opcode = 0x46c0; // nop (mov r8, r8)
			return instruction;
		}
