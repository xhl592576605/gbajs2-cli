const { GameBoyAdvanceRenderProxy } = require('./video/proxy');
const { GameBoyAdvanceSoftwareRenderer } = require('./video/software');
...
class GameBoyAdvanceVideo {
	...
	clear() {
		// Check if cpu is available before calling renderPath.clear
		if (this.cpu && this.cpu.mmu) {
	...
	setBacking(backing) {
		// In Node.js environment, backing may be null
		var pixelData;
		if (backing && typeof backing.createImageData === 'function') {
			pixelData = backing.createImageData(
				this.HORIZONTAL_PIXELS,
				this.VERTICAL_PIXELS
			);
			this.context = backing;
		} else {
			// Create a mock ImageData for Node.js environment
			var data = new Uint8ClampedArray(this.HORIZONTAL_PIXELS * this.VERTICAL_PIXELS * 4);
			// Fill with white pixels (RGBA)
